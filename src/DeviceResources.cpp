...
void DeviceResources::CreateDeviceResources()
{
    ...
    if (m_options & c_AllowTearing)
    {
        ...
        auto factory5{ m_dxgiFactory.as<IDXGIFactory5>() };
    ...
    auto d3dInfoQueue{ m_d3dDevice.as<ID3D12InfoQueue>() };
#ifdef _DEBUG
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(m_commandQueue.put())));
    ...
    ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&rtvDescriptorHeapDesc, IID_PPV_ARGS(m_rtvDescriptorHeap.put())));
    ...
    if (m_depthBufferFormat != DXGI_FORMAT_UNKNOWN)
    {
        ...
        ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&dsvDescriptorHeapDesc, IID_PPV_ARGS(m_dsvDescriptorHeap.put())));
    ...
    {
        ThrowIfFailed(m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(m_commandAllocators[n].put())));
    ...
    ThrowIfFailed(m_d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_commandAllocators[0].get(), nullptr, IID_PPV_ARGS(m_commandList.put())));
    ThrowIfFailed(m_commandList->Close());

    m_commandList->SetName(L"DeviceResources");

    // Create a fence for tracking GPU execution progress.
    ThrowIfFailed(m_d3dDevice->CreateFence(m_fenceValues[m_backBufferIndex], D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(m_fence.put())));
...
void DeviceResources::CreateWindowSizeDependentResources()
{
    ...
    for (UINT n = 0; n < m_backBufferCount; n++)
    {
        ThrowIfFailed(m_swapChain->GetBuffer(n, IID_PPV_ARGS(m_renderTargets[n].put())));
...
void DeviceResources::HandleDeviceLost()
{
...
#ifdef _DEBUG
    {
        ...
        if (SUCCEEDED(DXGIGetDebugInterface1(0, IID_PPV_ARGS(dxgiDebug.put()))))
        {
...
{
    if (m_commandQueue && m_fence && bool{ m_fenceEvent })
    {
...
void DeviceResources::GetAdapter(IDXGIAdapter1** ppAdapter)
{
    *ppAdapter = nullptr;
    ...
    winrt::com_ptr<IDXGIAdapter1> adapter;
    auto factory6{ m_dxgiFactory.as<IDXGIFactory6>() };
